...
def GetSettlementInfoMonth(info,TradingDay,ExchangeID):
    selectsql = """
          select  [TradingDay] from [PreTrade].[dbo].[SettlementInfoMonth] where TradingDay='%s' and ExchangeID='%s' and IsFuture=1
    """
    ...
    if len(list1) == 0:
        ###爬取期货信息
        if ExchangeID=="DCE":
            GetDCEFutureMonthInfo(TradingDay,info)
        elif ExchangeID=="CZCE":
            GetCZCEFutureMonthInfo(TradingDay,info)
        elif ExchangeID=="SHFE":
            GetSHFEFutureMonthInfo(TradingDay,info)
        elif ExchangeID=="CFFEX":
            GetCFFEXFutureMonthInfo(TradingDay,info)
    else:
        print TradingDay, ExchangeID + "期货月统计数据已经存在"

    optselectsql = """
        select TradingDay from [dbo].[SettlementInfoMonth] where TradingDay='%s' and ExchangeID='%s' and IsFuture=0
    """
    ...
    if len(list1) == 0:
        if ExchangeID=="DCE":
            GetDCEOptionMonthInfo(TradingDay, info)
        elif ExchangeID=="CZCE":
            GetCZCEOptionMonthInfo(TradingDay, info)
        elif ExchangeID=="SHFE" and TradingDay.strftime("%Y-%m-%d")>="2018-09-01":
            GetSHFEOptionMonthInfo(TradingDay, info)
        elif ExchangeID=="CFFEX":
            GetCFFEXOptionDayInfo(TradingDay, info)
    else:
        print TradingDay,ExchangeID+ "期权月统计数据已经存在"
...
def GetCFFEXFutureMonthInfo(TradingDay, info):
    url = "http://www.cffex.com.cn/sj/hqsj/ytj/%s/index.xml" % (TradingDay.strftime("%Y%m%d")[0:6])
    print url
    header = {
    }
    sql = """
           INSERT INTO [PreTrade].[dbo].[SettlementInfoMonth]
    ...
    content = bs.findAll("monthlydata")
    ...
    for i in content:
        ...
        temp=[info.TradingDay,str(i.find("instrumentid").text).strip(),i.find("openprice").text,"CFFEX", 1,
                              i.find("highestprice").text, i.find("lowestprice").text, i.find("closeprice").text,
                               i.find("settlementprice").text, i.find("volume").text,
...
"""-------------------------------------------------上期所-----------------------------------------------------------------------------"""
def GetSHFEOptionMonthInfo(TradingDay, info):
    url="http://www.shfe.com.cn/data/dailydata/option/%smonth.dat"% (TradingDay.strftime("%Y%m"))
    print url
    header = {
    ...
    for i in contentdata:
        ...
        if str(i['INSTRUMENTID'].encode("utf-8")).strip().find("计") == -1:
            ...
            temp = [info.TradingDay, InstrumentID, i['OPENPRICE'], 'SHFE', 0, i['HIGHESTPRICE'], i['LOWESTPRICE'],
                    i['CLOSEPRICE'], i['SETTLEMENTPRICE'],i['VOLUME'], i['OPENINTEREST'],
...
def  GetSHFEFutureMonthInfo(TradingDay,info):
    url = "http://www.shfe.com.cn/data/dailydata/%smonth.dat" % (TradingDay.strftime("%Y%m"))
    print url
    header = {
        'Connection': 'keep-alive',
        'Cache-Control': 'no-cache',
    ...
    for i in contentdata:
        ...
        if str(i['INSTRUMENTID'].encode("utf-8")).strip().find("计")==-1 :
            ...
            temp=[info.TradingDay,InstrumentID,i['OPENPRICE'],'SHFE',1, i['HIGHESTPRICE'], i['LOWESTPRICE'],
                 i['CLOSEPRICE'], i['SETTLEMENTPRICE'],i['VOLUME'], i['OPENINTEREST'],i['OPENINTERESTCHG'],i['TURNOVER']]
            if temp[8] != "" and temp[4] == "" and temp[5] == "" and temp[6] == "" and temp[2]=="":
                temp[2] = temp[8]
                temp[5] = temp[8]
                temp[6] = temp[8]
...
""""------------------------------------郑商所------------------------------------------------------------------------------------------"""
def GetCZCEOptionMonthInfo(TradingDay, info):
    url="http://www.czce.com.cn/cn/DFSStaticFiles/Option/%s/%s/OptionDataMonth.htm"% (info.TradingDay[0:4], info.TradingDay.replace("-",""))
    print url
    header = {
...
def GetCZCEFutureMonthInfo(TradingDay,info):
    #http://www.czce.com.cn/cn/DFSStaticFiles/Future/2019/20190430/FutureDataMonth.htm
    url = "http://www.czce.com.cn/cn/DFSStaticFiles/Future/%s/%s/FutureDataMonth.htm" % (info.TradingDay[0:4], info.TradingDay.replace("-",""))
    print url
    header = {
        'Host': 'www.czce.com.cn',
        'Connection': 'keep-alive',
...
"""------------------------------大商所--------------------------------------------------------------------------------------------------"""
def GetDCEOptionMonthInfo(TradingDay, info):
    url = "http://www.dce.com.cn/publicweb/quotesdata/monthQuotesCh.html?monthQuotes.variety=all&monthQuotes.trade_type=1&year=%s&month=%s&day=%s"
    header = {
        'Connection': 'keep-alive',
        'Cache-Control': 'no-cache',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8',
    ...
    optsql = """
        INSERT INTO [dbo].[SettlementInfoMonth]([TradingDay],[InstrumentID],[ExchangeID],[IsFuture],[OpenPrice],[HighestPrice],[LowestPrice],[ClosePrice],[SettlementPrice],
...
def GetDCEFutureMonthInfo(TradingDay, info):
    url="http://www.dce.com.cn/publicweb/quotesdata/monthQuotesCh.html?monthQuotes.variety=all&monthQuotes.trade_type=0&year=%s&month=%s&day=%s"
...
def GetMonthLastTradingDay(monthstartdate,monthenddate,t):
    """找出一个月的最后一个交易日"""
    while monthenddate>monthstartdate:
        if t.IsTradingDayFuture(monthenddate.strftime("%Y%m%d")):
            return monthenddate
        monthenddate = monthenddate - datetime.timedelta(days=1)
    raise Exception