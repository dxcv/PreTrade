...
#读取数据
def ReadData(file,fileDirectory,mysql):
    ...
    if file.startswith("DC"):
        InstrumentId=file.replace(".csv","")[2:]
    else:
        InstrumentId=file.replace(".csv","")
    col=[]
    try:
        ...
        print  "u'源数据文件不存在,或请先关闭文件",e.message
        return
    length = len(csv_data) - 1
    backup_name = file.replace(".csv", "_backup.csv")
